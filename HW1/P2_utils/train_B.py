...
TRANSFORM_IMG = [
                    ...
                    T.Compose([T.RandomHorizontalFlip(p=0.5), T.RandomVerticalFlip(p=0.5)]),
...
train_valid_loader = DataLoader(ImgDataset(os.path.join("p2_data", "train"),  "valid", 'cache.pkl'), batch_size=batch_size, shuffle=False, num_workers=8, persistent_workers=True)
...
def train(model, epochs=50, lr=0.01, model_prefix="", run_parallel=False):
    ...
    for epoch in range(epochs):
        ...
        for i, (x, y, _) in enumerate(pbar):
            ...
            # if step % 2000 == 0:
            #   torch.save(resnet, f'./{model_prefix}_{epoch}_{i}_{step}.pt')
        if epoch % 5 == 0:
            accuracy = evaluate(model, train_valid_loader, 'train')
...
model = DeepLab(7).cuda()

# print(model)
train(model, 20, 5e-5, "deeplabv3_resnet101_20", True)
